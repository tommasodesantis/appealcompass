...
test("print route renders required Assessor packet sections", async () => {
  ...
  expect(html).toContain("This evidence packet was generated by Appeal Compass on");
  expect(html).toContain("to support a residential property-tax appeal for PIN");
  expect(html).toContain("Subject Property");
  expect(html).toContain("Evidence level");
...
test("print route suppresses user-facing calendar staleness warnings", async () => {
  ...
  expect(html).toContain("All deadlines and data should be verified at the official sources cited");
  expectNoBannedText(html);
});
