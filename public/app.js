var y=document.querySelector("#app");if(!y)throw new Error("Missing app root.");var A=y,$=new Intl.NumberFormat("en-US",{style:"currency",currency:"USD",maximumFractionDigits:0}),w="Appeal Compass is designed only for individual residential homeowners appealing their own home; entity-owned, commercial, and association properties are not supported and generally require an attorney.";function i(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function o(e){return e===null?"Not available":$.format(e)}function l(e,n=0){return e===null?"Not available":e.toLocaleString("en-US",{maximumFractionDigits:n})}async function T(e){let n=await fetch(e,{headers:{accept:"application/json"}}),t=await n.json();if(!n.ok||typeof t=="object"&&t&&"ok"in t&&t.ok===!1){let s=t.error?.message??"The request failed.";throw new Error(s)}return t}function m(e,n){let t=new FormData(e).get(n);return typeof t=="string"?t.trim():""}function L(e,n){let t=["jurisdiction","venue","ownershipType","assessorAppealFiled","assessorDecisionReceived","borAppealFiled","borDecisionReceived","borDecisionDate","purchasePrice","purchaseDate","appraisalValue","appraisalDate","actualSqft","actualAv","actualImprovementAv"];for(let s of t){let a=m(n,s);a&&e.set(s,a)}}function f(e){return`<section class="progress" aria-live="polite"><p>${i(e)}</p></section>`}function S(){return`<svg aria-hidden="true" class="github-mark" viewBox="0 0 16 16" width="20" height="20">
    ...
    <a class="footer-icon-link" href="https://github.com/tommasodesantis/appealcompass" target="_blank" rel="noreferrer">${S()}<span>View on GitHub</span><span class="sr-only"> (opens in new tab)</span></a>
  ...
  </footer>`}function q(){let e=["Looking up your property...","Fetching assessment history...","Finding similar homes...","Building the evidence summary..."],n=0,t=e[0]??"",s=document.querySelector("#progress");s&&(s.innerHTML=f(e[n]??t));let a=window.setInterval(()=>{n=(n+1)%e.length;let r=document.querySelector("#progress");r&&(r.innerHTML=f(e[n]??t))},650);return()=>window.clearInterval(a)}function M(){A.innerHTML=`
    ...
    <div id="results"></div>
    ${E()}
  `}function c(e){let n=document.querySelector("#form-error");n&&(n.innerHTML=e?`<section class="error inline-error" role="alert">${i(e)}</section>`:"")}function u(e,n){let t=new FormData(e).get(n);return typeof t=="string"?t:""}function p(e,n,t){let s=e.querySelector(`[data-conditional="${n}"]`);if(s){s.hidden=!t;for(let a of Array.from(s.querySelectorAll("input, select, textarea"))){if(!(a instanceof HTMLInputElement||a instanceof HTMLSelectElement||a instanceof HTMLTextAreaElement))continue;let r=a;r.disabled=!t,r.required=t,t||(r instanceof HTMLInputElement&&r.type==="radio"?r.checked=!1:r.value="")}}}function v(e){let n=u(e,"assessorAppealFiled")==="yes",t=u(e,"borAppealFiled")==="yes",s=u(e,"borDecisionReceived")==="yes";p(e,"assessorDecision",n),p(e,"borDecision",t),p(e,"borDecisionDate",t&&s)}function H(e){return c(""),v(e),e.reportValidity()?m(e,"ownershipType")!=="individual"?(c(w),!1):!0:!1}function P(e){return e.length===0?"":`<section class="warnings" aria-label="Warnings"><h2>Warnings</h2><ul>${e.map(n=>`<li>${i(n)}</li>`).join("")}</ul></section>`}function D(e){let n=e.routing,t=n.officialUrl?`<a href="${i(n.officialUrl)}" target="_blank" rel="noreferrer">Verify at the official source before filing</a>`:"";if(!n.deadline)return`<p>No computed deadline. ${t}</p>`;let s=n.daysRemaining===null?"":` ${n.daysRemaining>=0?`${n.daysRemaining} days remaining.`:`${Math.abs(n.daysRemaining)} days past the computed deadline.`}`;return`<p><strong>Deadline:</strong> ${i(n.deadline)}.${i(s)} ${t}</p>`}function F(e){let n=e.evidence.comparableAnalysis,t=n.exhibit.map(a=>{let r=n.profileLabel.includes("Assessor")||e.routing.venue==="closed"?a.comparable.av:a.comparable.improvementAv;return`<tr>
        <td>${i(a.comparable.pinFormatted)}</td>
        <td>${l(a.comparable.buildingSqft)}</td>
        <td>${i(a.comparable.yearBuilt??"Not available")}</td>
        <td>${o(r)}</td>
        <td>${o(a.avPerSqft)}</td>
      </tr>`}).join(""),s=t.length===0?"<p>No lower-assessed comparable exhibit is available from the current public data.</p>":`<div class="table-wrap"><table>
    ...
    <p><strong>Tier:</strong> ${i(e.evidence.tier)}. ${i(e.evidence.tierMessage)}</p>
    <p class="hint">The tier is a rough screen of how much public data supports spending time on an appeal.</p>
    <p><strong>Comparable profile:</strong> ${i(n.profileLabel)} using ${i(n.metricLabel)}.</p>
    <p class="hint">Comparable analysis matters because uniformity appeals compare your assessment to similar homes.</p>
    <p>${i(n.note)}</p>
    <p><strong>Pool:</strong> ${l(n.poolSize)} similar homes, ${i(n.scope??"no scope")}; subject ${i(n.metricLabel)}/sqft ${o(n.subjectAvPerSqft)}; median ${o(n.medianAvPerSqft)}; gap ${l(n.gapPct,1)}%.</p>
    ${s}
    <h3>Arguments</h3>
  ...
  </section>`}function k(e,n){let t=e.case.parcel,s=[t.address,t.city,t.zipCode].filter(Boolean).join(", "),a=[e.case.userEvidence.actualSqft?`Actual sqft ${l(e.case.userEvidence.actualSqft)}`:"",e.case.userEvidence.actualAv?`Actual AV ${o(e.case.userEvidence.actualAv)}`:"",e.case.userEvidence.actualImprovementAv?`Actual improvement AV ${o(e.case.userEvidence.actualImprovementAv)}`:""].filter(Boolean),r=new URLSearchParams(n);r.set("pin",t.pin),e.demo&&r.set("demo","1");let g=document.querySelector("#results");g&&(g.innerHTML=`
    ...
    <section class="panel" aria-labelledby="step-three">
      ...
      ${D(e)}
      <ul>${e.routing.reasoning.map(d=>`<li>${i(d)}</li>`).join("")}</ul>
    </section>

    <section class="subject panel">
      <h2>Subject property</h2>
      <dl>
        <div><dt>PIN</dt><dd>${i(t.pinFormatted)}</dd></div>
        ${s?`<div><dt>Address</dt><dd>${i(s)}</dd></div>`:""}
        <div><dt>Class / township</dt><dd>${i(t.propertyClass)} / ${i(t.townshipName)}</dd></div>
    ...
    <section class="panel" aria-labelledby="step-five">
      ...
      <ul>${e.venue.checklist.map(d=>`<li>${i(d)}</li>`).join("")}</ul>
      <a class="button-link" href="/print?${r.toString()}">Print / Save as PDF</a>
    </section>

    ${P(e.warnings)}
  `)}function b(){c("");for(let e of["#results","#address-results"]){let n=document.querySelector(e);n&&(n.innerHTML="")}}async function R(e){b();let n=q();try{let t=await T(`/api/case?${e.toString()}`);b(),k(t,e)}catch(t){let s=document.querySelector("#results");s&&(s.innerHTML=`<section class="error" role="alert">${i(t instanceof Error?t.message:"The case could not be loaded.")}</section>`)}finally{n();let t=document.querySelector("#progress");t&&(t.innerHTML="")}}async function C(e){if(!H(e))return;let n=new URLSearchParams,t=m(e,"pin");if(L(n,e),t){n.set("pin",t),await R(n);return}let s=document.querySelector("#results");s&&(s.innerHTML='<section class="error" role="alert">Enter a PIN.</section>')}function I(){let e=document.querySelector("details.evidence");if(e)for(let n of Array.from(e.querySelectorAll("[data-evidence-input]")))(n instanceof HTMLInputElement||n instanceof HTMLTextAreaElement)&&(n.value="")}M();var h=document.querySelector("#case-form");h&&v(h);document.addEventListener("submit",e=>{let n=e.target;n instanceof HTMLFormElement&&n.id==="case-form"&&(e.preventDefault(),C(n))});document.addEventListener("change",e=>{let n=e.target;if(n instanceof HTMLInputElement||n instanceof HTMLSelectElement||n instanceof HTMLTextAreaElement){let t=n.form;t?.id==="case-form"&&(c(""),v(t))}});document.addEventListener("click",e=>{let n=e.target;n instanceof HTMLElement&&n.id==="clear-evidence"&&I()});document.documentElement.dataset.enhanced="true";
