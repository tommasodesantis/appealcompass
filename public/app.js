var v="";var I=new TextEncoder;function _(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function V(t){return t&&typeof t=="object"&&"value"in t?t:{value:t}}function j(t){let e=t+1,n="";for(;e>0;){let s=(e-1)%26;n=String.fromCharCode(65+s)+n,e=Math.floor((e-1)/26)}return n}function B(t,e,n){let s=V(t),r=`${j(n)}${e}`,a=s.style===void 0?"":` s="${s.style}"`;if(typeof s.value=="number"&&Number.isFinite(s.value))return`<c r="${r}"${a}><v>${s.value}</v></c>`;let i=s.value===null||s.value===""?"Not available":s.value;return`<c r="${r}" t="inlineStr"${a}><is><t>${_(i)}</t></is></c>`}function O(t){return`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetViews><sheetView workbookViewId="0"/></sheetViews>
  <sheetFormatPr defaultRowHeight="15"/>
  <cols><col min="1" max="8" width="22" customWidth="1"/></cols>
  <sheetData>${t.map((n,s)=>`<row r="${s+1}">${n.map((r,a)=>B(r,s+1,a)).join("")}</row>`).join("")}</sheetData>
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  </footer>`}function ce(){return`<a class="header-icon-link" href="https://github.com/tommasodesantis/appealcompass" target="_blank" rel="noreferrer">${N()}<span>View on GitHub</span><span class="sr-only"> (opens in new tab)</span></a>`}function pe(){let t=g?"":" disabled",e=g?`<div class="cf-turnstile" data-sitekey="${o(v)}"></div>`:'<p class="hint">Problem reporting is disabled until the Turnstile site key is configured.</p>';return`<section id="report-panel" class="report-panel" hidden>
  ...
  `}function h(t){let e=document.querySelector("#form-error");e&&(e.innerHTML=t?`<section class="error inline-error" role="alert">${o(t)}</section>`:"")}function m(t,e=!1){let n=document.querySelector("#report-status");n&&(n.innerHTML=t?`<p class="${e?"error inline-error":"notice inline-error"}">${o(t)}</p>`:"")}function fe(){let t=document.querySelector("#report-panel"),e=document.querySelector("#report-context");if(!t)return;e&&d&&(e.value=`PIN ${d.case.parcel.pinFormatted}; venue ${d.routing.venue}; generated ${d.generatedAt}`),m(g?"":"Problem reporting is disabled until the Turnstile site key is configured.",!0),t.hidden=!1,t.querySelector("select, textarea, input, button")?.focus()}function be(){let t=document.querySelector("#report-panel");t&&(t.hidden=!0)}function x(t,e){let n=new FormData(t).get(e);return typeof n=="string"?n:""}function w(t,e,n){let s=t.querySelector(`[data-conditional="${e}"]`);if(s){s.hidden=!n;for(let r of Array.from(s.querySelectorAll("input, select, textarea"))){if(!(r instanceof HTMLInputElement||r instanceof HTMLSelectElement||r instanceof HTMLTextAreaElement))continue;let a=r;a.disabled=!n,a.required=n,n||(a instanceof HTMLInputElement&&a.type==="radio"?a.checked=!1:a.value="")}}}function S(t){let e=x(t,"assessorAppealFiled")==="yes",n=x(t,"borAppealFiled")==="yes",s=x(t,"borDecisionReceived")==="yes";w(t,"assessorDecision",e),w(t,"borDecision",n),w(t,"borDecisionDate",n&&s)}function ge(t){return h(""),S(t),t.reportValidity()?k(t,"ownershipType")!=="individual"?(h(re),!1):!0:!1}function he(t){return t.length===0?"":`<section class="warnings" aria-label="Warnings"><h2>Warnings</h2><ul>${t.map(e=>`<li>${T(e)}</li>`).join("")}</ul></section>`}function ve(){return`<section class="panel" aria-labelledby="exemptions">
    ...
    ${he(t.warnings)}
  `)}function M(){d=null,h("");for(let t of["#results","#address-results"]){let e=document.querySelector(t);e&&(e.innerHTML="")}}async function Ae(t){M();let e=ue(await de());try{let n=await U(`/api/case?${t.toString()}`);M(),we(n,t)}catch(n){let s=document.querySelector("#results");s&&(s.innerHTML=`<section class="error" role="alert">${o(n instanceof Error?n.message:"The case could not be loaded.")}</section>`)}finally{e();let n=document.querySelector("#progress");n&&(n.innerHTML="")}}async function Te(t){if(!ge(t))return;let e=new URLSearchParams,n=k(t,"pin");if(ie(e,t),n){e.set("pin",n),await Ae(e);return}let s=document.querySelector("#results");s&&(s.innerHTML='<section class="error" role="alert">Enter a PIN.</section>')}async function ke(t){if(!g){m("Problem reporting is disabled until the Turnstile site key is configured.",!0);return}if(!t.reportValidity())return;let e=new FormData(t),n=e.get("cf-turnstile-response");m("Submitting report...");let s=await fetch("/api/report",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({category:e.get("category"),description:e.get("description"),context:e.get("context"),turnstileToken:typeof n=="string"?n:""})}),r=await s.json();if(!s.ok||!r.ok){m(r.ok?"The report could not be submitted.":r.error?.message??"The report could not be submitted.",!0);return}m(`Report submitted: ${r.issueUrl}`),t.reset()}function Se(){let t=document.querySelector("details.evidence");if(t)for(let e of Array.from(t.querySelectorAll("[data-evidence-input]")))(e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement)&&(e.value="")}function $(t=null){for(let e of Array.from(document.querySelectorAll(".tooltip-toggle"))){if(e===t)continue;let n=e.getAttribute("aria-describedby"),s=n?document.getElementById(n):null;e.setAttribute("aria-expanded","false"),s&&(s.hidden=!0)}}function $e(t){let e=t.getAttribute("aria-describedby"),n=e?document.getElementById(e):null;if(!n)return;let s=t.getAttribute("aria-expanded")!=="true";$(s?t:null),t.setAttribute("aria-expanded",String(s)),n.hidden=!s}function Ee(){if(!d)return;let t=q(d),e=new ArrayBuffer(t.byteLength);new Uint8Array(e).set(t);let n=new Blob([e],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}),s=URL.createObjectURL(n),r=document.createElement("a");r.href=s,r.download=P(d),document.body.appendChild(r),r.click(),r.remove(),URL.revokeObjectURL(s)}me();var R=document.querySelector("#case-form");R&&S(R);document.addEventListener("submit",t=>{let e=t.target;e instanceof HTMLFormElement&&e.id==="case-form"&&(t.preventDefault(),Te(e)),e instanceof HTMLFormElement&&e.id==="report-form"&&(t.preventDefault(),ke(e))});document.addEventListener("change",t=>{let e=t.target;if(e instanceof HTMLInputElement||e instanceof HTMLSelectElement||e instanceof HTMLTextAreaElement){let n=e.form;n?.id==="case-form"&&(h(""),S(n))}});document.addEventListener("click",t=>{let e=t.target;if(e instanceof HTMLElement){let n=e.closest(".tooltip-toggle");if(n){$e(n);return}$()}e instanceof HTMLElement&&e.id==="clear-evidence"&&Se(),e instanceof HTMLElement&&e.id==="download-comps"&&Ee(),e instanceof HTMLElement&&e.id==="report-problem"&&fe(),e instanceof HTMLElement&&(e.id==="close-report"||e.id==="report-panel")&&be()});document.addEventListener("keydown",t=>{t.key==="Escape"&&$()});document.documentElement.dataset.enhanced="true";
